...
def train_nn(nn_params_path="nn_params.pt"):
    # torch set seed
    torch.manual_seed(12345)

    train_inputs = np.linspace(-2 * np.pi, 2 * np.pi, int(1e5))
    train_targets = 10 * np.sin(0.25 * train_inputs) ** 2 + 0.01 * np.random.randn(
        *train_inputs.shape
    )

    train_inputs = torch.tensor(train_inputs, dtype=torch.float32).unsqueeze(1)
    ...
    optimizer = torch.optim.SGD(nn_fun.parameters())
    loss_fn = torch.nn.MSELoss()

    for epoch in range(50):

        for i, data in enumerate(train_set):
            train_inputs_batch, train_targets_batch = data
            optimizer.zero_grad()
            outputs_batch = nn_fun(train_inputs_batch)
            loss = loss_fn(outputs_batch, train_targets_batch)
            loss.backward()
            optimizer.step()
    ...
    test_inputs = torch.tensor(
        np.linspace(-2 * np.pi, 2 * np.pi, int(1e3)), dtype=torch.float32
    ).unsqueeze(1)

    test_outputs = nn_fun(test_inputs)

    print(
        f"Test loss: {loss_fn(test_outputs, 10 * torch.sin(0.25 * test_inputs) ** 2)}"
    )

    nn_params = nn_fun.state_dict()
    torch.save(nn_params, nn_params_path)


class DoubleIntegratorWithLearnedDynamics:
    def __init__(self, learned_dyn=None, name="wr"):
    ...
    def model(self):
        ...
        f_expl = cs.vertcat(s_dot, u) + B @ res_model
